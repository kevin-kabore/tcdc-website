...
gulp.task('js', function() {
  ...
  gulp
    .src(['src/js/lib/**/*.js', 'src/js/**/*.js']) // path to your files
    .pipe(
      sort(function(a, b) {
...
gulp.task('pages', function(callback) {
  //validate and copy html and php pages
  return gulp
    .src('src/**/*.+(php|html)')
    .pipe(plumber({ errorHandler: onError })) //if the validator crashes, fail gracefully
    .pipe(phplint())
...
gulp.task('build', function(callback) {
  //do a full build for the site
  runSequence(
    'clean:build',
    ['css', 'js', 'images', 'fonts', 'media', 'pages'],
    callback
  );
});
...
gulp.task('watch', ['browser-sync'], function() {
  ...
  gulp
    .watch(['src/scss/**/*.scss', 'src/css/**/*.css'])
    .on('change', function() {
      gulp.start('css');
      browserSync.reload();
    });
